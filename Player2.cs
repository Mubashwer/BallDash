...
namespace Project
{
    ...
    public class Player2 : GameObject
    {
        ...
        public void SetCubeRenderTarget(RenderTargetCube renderTargetCube, CubeMapFace face)
        {
            ...
            var renderTargetsBefore = device.GetRenderTargets(out depthStencilBefore);
            device.SetRenderTargets(depthStencilBefore, renderTargetCube.RenderTargetView[ViewType.Full, 0, 0]);
            //device.SetRenderTargets(renderTargetCube, face);
            //return new RenderTargetCube
            //SharpDX.Direct3D11.RenderTargetView(renderTargetCube, face);
        }

        public override void RenderSceneToTextureCube(ref Matrix projection)
        {
            //Matrix proj = Matrix.PerspectiveFovLH(fov, aspect, zner, zfar);
            //proj.M33 *= -1;
            //proj.M34 *= -1;
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                RenderTargetCube renderTargetCube = new RenderTargetCube(null, null, null);
                // Set the cubemap render target, using the selected face
                SetCubeRenderTarget(renderTargetCube, cubeMapFace);
                envMap = renderTargetCube.ShaderResourceView;
                //SetCubeRenderTarget(renderTargetCube, cubeMapFace);
